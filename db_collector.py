...
def flatten_pair(detail: dict) -> dict:
    ...
    twitter_url  = next((s["url"] for s in info.get("socials", []) if s["type"] == "twitter"), None)
    telegram_url = next((s["url"] for s in info.get("socials", []) if s["type"] == "telegram"), None)
...
async def fetch_updated_detail(pair_address: str, session: aiohttp.ClientSession) -> dict:
    ...
    try:
        async with aiosqlite.connect(db_file) as db:
            ...
            for idx, col in enumerate(HEADERS):
                ...
                if (old_val in (None, '', 0) or old_val == '') and new_val not in (None, '', 0):
            ...
            if update_cols:
                ...
                await db.commit()
                print(f"🔄 Patched {len(update_cols)} missing fields for {row['pairAddress']}")
    except Exception as e:
...
async def complete_detail(
    ...
    print(f"⚠️  Still incomplete for {addr} after {max_retries} retries, proceeding anyway")
    return detail
    ...
    async with LOCK:
        ...
        for idx, row in to_classify.iterrows():
            ...
            if age >= CLASSIFY_DELAY:
                ...
                if not data or any(k not in data for k in ("priceUsd", "fdv", "liquidity")):
                    df.at[idx, "worthy"] = 0
                else:
                    if (
                        float(data["priceUsd"]) > row["priceUsd"] * price_mul and
                        data["fdv"]              > row["fdv"]      * fdv_mul and
                        data["liquidity"]["usd"] > row["liquidity_usd"] * liq_mul and
                        data["fdv"]              > fdv_min and
                        data["liquidity"]["usd"] > liq_min
                    ):
                        df.at[idx, "worthy"] = 1
                    else:
                        df.at[idx, "worthy"] = 0
        ...
        while True:
            ...
            if new_addrs:
                ...
                if valid:
                    ...
                    seen_addresses.update(p["pairAddress"] for p in valid if p.get("pairAddress"))
            if RECLASSIFY:
